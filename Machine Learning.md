...
### Week4 决策树
#### 决策树模型
**决策树概念**：通过一步步决策得到最终结果的树。简单来说，通过对一系列问题进行if/else的推导，最终实现相关决策

一颗完整的决策树包含***三部分***：
- 根节点：就是树最顶端的节点，比如图中的“耳朵形状”
- 叶子节点：树最底部的那些节点，也就是决策结果，是猫还是不是猫
- 决策节点：除了叶子结点，都是决策节点

![](https://cdn.jsdelivr.net/gh/IvenStarry/Image/MarkdownImage/202408190935083.png)
**算法工作**：从所有可能的决策树中，尝试选择一个希望在训练集上表现良好的树，然后泛化到新数据，例如交叉验证和测试集
![](https://cdn.jsdelivr.net/gh/IvenStarry/Image/MarkdownImage/202408190935789.png)

#### 学习过程
**决策树创建过程**：
1. 决定根节点的特征选择
2. 只关注决策树左侧部分(有时称为左侧分支)，以决定将哪些节点放在左侧
3. 根据分类结果正确率创建相应的叶子节点
4. 在右侧重复该过程
![](https://cdn.jsdelivr.net/gh/IvenStarry/Image/MarkdownImage/202408191007094.png)

决策树模型要**解决的问题**：
1. 如何选择在每个节点分裂的特征？
   - 最大化纯度(最小化杂质)，也就是让经过节点后的样本分类准确率更高，分类区分度更明显
    ![](https://cdn.jsdelivr.net/gh/IvenStarry/Image/MarkdownImage/202408191007887.png)
2. 何时停止分裂？
   - 当一个节点100%为一个类别(节点左侧100%为猫，右侧100%为非猫)
   - 当分裂一个节点会导致决策树超过最大深度(深度参数可以设置，深度：从根节点到特定节点的跳数)
   ![](https://cdn.jsdelivr.net/gh/IvenStarry/Image/MarkdownImage/202408191009289.png)
   - 当信息增益(熵值的减小)低于一个阈值(为了保持树更小并且防止过拟合，这些节点存在收益很小)
   - 当一个节点的样本数量低于某个阈值(样本数只剩3个，将决策节点转为叶子节点)
   ![](https://cdn.jsdelivr.net/gh/IvenStarry/Image/MarkdownImage/202408191010670.png)

#### 纯度
**熵**(entropy)：杂质的度量，样本的混乱程度，用符号H表示
![](https://cdn.jsdelivr.net/gh/IvenStarry/Image/MarkdownImage/202408191509385.png)
图像释义：横轴是p_1(样本中猫的比例)，纵轴是熵的值，当p_1越接近0.5，熵值越大(接近1)，纯度越低，更混乱
![](https://cdn.jsdelivr.net/gh/IvenStarry/Image/MarkdownImage/202408191510722.png)
**熵的公式**： $H(p_i) = - p_i \cdot log_2(p_i) - (1 - p_i) \cdot log2(1 - p_i)$
...
H：熵值 w：示例权重 p_i：示例比值
![](https://cdn.jsdelivr.net/gh/IvenStarry/Image/MarkdownImage/202408191532128.png)

#### 整合
**建立决策树过程**：
1. 从树的根节点处的所有训练示例开始
2. 计算所有可能特征的信息增益，并选择要分裂的特征，从而提供最高的信息增益
3. 选择这个特征后将数据集分裂为两个子集，并创建树的左右分支，并将训练示例发送至左侧或右侧分支，具体取决于样本特征
4. 在左右分支上重复分裂过程，直至达到停止分裂条件

**递归算法**：是一种直接或者间接调用自身方法的算法，在定义自身的同时又出现自身的直接或间接调用。
*e.g\.*：构建决策树的方式是通过构建较小的子决策树，然后将它们放在一起形成总
![](https://cdn.jsdelivr.net/gh/IvenStarry/Image/MarkdownImage/202408191551621.png)

#### 独热编码One-hot
**独热编码One-hot**：采用N位状态寄存器来对N个状态进行编码，每个状态都有独立的寄存器位，并且在任意时候只有一位有效，其中一个特征始终取值为1，即热特征，这种方法可以让决策树采用两个以上离散值的特征，不仅适用于决策树学习，还可以用于神经网络
![](https://cdn.jsdelivr.net/gh/IvenStarry/Image/MarkdownImage/202408191606979.png)
如果一个分类特征可以取k个可能的值，那么我们将替换它通过k个二进制特征来实现，这些特征取0或1

#### 连续可变特征
考虑分裂的不同阈值，计算信息增益，并决定在该连续值特征提供尽可能高的信息增益时进行分裂
![](https://cdn.jsdelivr.net/gh/IvenStarry/Image/MarkdownImage/202408191636598.png)

#### 回归树
**回归树概念**：当预测输出为连续值而非离散值(二元分类)时，用作回归任务的决策树被称为回归树
![](https://cdn.jsdelivr.net/gh/IvenStarry/Image/MarkdownImage/202408191709272.png)
构建回归树时，选择**分裂特征的标准**不是想要减少熵，而是尝试**减少**每个数据子集Y的**权重方差**，减少值越大，表示经分裂后两个分支之间的离散程度越高，分类效果更好，区分度越大
![](https://cdn.jsdelivr.net/gh/IvenStarry/Image/MarkdownImage/202408191705113.png)
